...
class DerivAPI:
    ...
    async def send_request(self, request: Dict) -> Dict:
        ...
        except (websockets.exceptions.WebSocketException, json.JSONDecodeError, ConnectionError) as e:
            logging.error(f"API request error: {e}")
            return {}
        except Exception as e:
            logging.error(f"Unexpected API request error: {e}")
            return {}
    ...
    async def _get_candles_async(self, symbol: str, granularity: int = 60, count: int = 100) -> pd.DataFrame:
        try:
            ...
            logging.error(f"Unexpected candles error for {symbol}: {e}")
            return pd.DataFrame()
    ...
    async def buy_contract(self, symbol: str, contract_type: str, amount: float, 
        ...
        try:
            ...
            if response.get('buy'):
                ...
                logging.error(f"Contract buy error: {response['error']}")
            return response
        except Exception as e:
            logging.error(f"Unexpected contract buy error: {e}")
            return {}
...
class DerivDonkeyBot:
    ...
    async def place_contract(self, signal: Dict) -> bool:
        try:
            ...
            logging.error(f"Contract placement data error: {e}")
        except Exception as e:
    ...
    async def monitor_positions(self):
        try:
            ...
            logging.error(f"Position monitoring data error: {e}")
        except Exception as e:
